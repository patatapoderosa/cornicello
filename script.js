...
function startMarquee() {
  if (!marquee || !marqueeTrack || !marqueeSet || window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    return;
  }
  ...
  const waitForImages = () => Promise.allSettled([...marqueeSet.querySelectorAll("img")].map((image) => {
